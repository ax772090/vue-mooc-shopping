...
<script>
	...
	export default {
		...
		computed: {
			//这里通过选中的商品数量是否和购物车里面的数量相等判断是否是全选
			checkAllFlag() {
			...
			totalPrice() {
				...
				this.cartList.forEach((item, index) => {
					if (item.checked === "1") {
						totalPri += parseInt(item.productNum) * parseFloat(item.salePrice);
					}
				});
				return totalPri;
			}
		...
		methods: {
			...
			delCartConfirm(productId) {
				...
				this.modalConfirm = true;//点击删除按钮弹出提示框
			},
			closeModal() {
			...
			editCart(flag, item) {
				if (flag === 'add') {
					item.productNum++;
				} else if (flag === 'minus' && item.productNum > 1) {
					item.productNum--;
			...
			toggleCheckAll() {
				//this.checkAllFlag =!this.checkAllFlag;//这个属性因为设置成实时计算属性了，所以这里没法改变
